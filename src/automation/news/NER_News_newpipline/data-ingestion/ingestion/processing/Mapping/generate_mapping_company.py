...
#from ingestion.processing.VDB_Similarity_Search.Model import NVEmbed
from ingestion.processing.VDB_Similarity_Search.VDB_Common import MilvusDB
...
from ingestion.models import SentenceSplit,SentenceSplitOut, SelectedSentence, NERMapped
...
class SimilarityMapping(MilvusDB):
    ...
    def transform_data_to_results_dict(self, find_cursor: list):
        ...
        @return: list[SentenceSplit] - A list of SentenceSplitOut objects.
        """
        results = []
    ...
    #     """
    #     results_dict = {"_id": [], 'article_id': [], 'News_id': [], "Sentence_id": [], "Output_sentence1": [], "Output_sentence2": [],
    #                     "Companies_econs": [], "Date": [], "Title": []}
    #     for result in tqdm(find_cursor):
    #         results_dict['_id'].append(result['_id'])
    #         results_dict['article_id'].append(result['article_id'])
    #         results_dict['News_id'].append(result['News_id'])
    ...
    def get_sentence_split_data(self):
        ...
        try:
            ...
            raise Exception(f"Mongo read for new documents failed: {e}")
    ...
    def calculate_similarity_helper(
        ...
        def _compute_similarity_for_each_mode(
            ...
            if isinstance(ner_comp, list):
                ner_comp = " ".join(ner_comp) if len(ner_comp) > 0 else ""

            if isinstance(name_to_map, list):
                name_to_map = " ".join(name_to_map) if len(name_to_map) > 0 else ""
    ...
    def calculate_similarity(self, article: SentenceSplitOut, lower_bound: int):
        """
        ...
        for q in range(len(input_article["Companies_econs"])):
            if len(input_article["Output_sentence2"][q]) != 0:
                if input_article["Output_sentence2"][q][8] == "C":
                    ...
                    # _id = create_id(DEFAULT_NER_MAPPING_COLLECTION, ner_comp_similarity2)
    ...
    def batch_helper(self, batch: list[SentenceSplitOut]):
        """
    ...
    def run(self, message_data: list[SentenceSplit]):
        # try:
        if len(message_data) > 0:
            split_sentence_records = [record.to_out_format() for record in message_data]
        else:
            split_sentence_records = self.get_sentence_split_data()

        if len(split_sentence_records) == 0:
            return []
        batches = list(split_iter(split_sentence_records, 20))
        with parallel_backend("threading", n_jobs=5):
            parallel_results = Parallel(n_jobs=5)(
                delayed(self.batch_helper)(batch)
                for batch in tqdm(batches, desc="Processing batches", total=len(batches), dynamic_ncols=True)
            )

        return parallel_results

        # except Exception as e:
        #     raise Exception(f"Failed to run similiarity mapping: {e}")
