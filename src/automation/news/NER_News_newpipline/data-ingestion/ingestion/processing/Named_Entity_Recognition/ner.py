...
from ingestion.db import create_id, database, pull_mongo_data
from ingestion.models import Article, NEROut
from ingestion.processing.Named_Entity_Recognition.ner_ruler import NER_Ruler
...
class NER_TextProcessor(NER_Ruler):
    ...
    def generate_text_sentence(self, record: Article) -> NEROut:
        ...
        if not isinstance(text, (str, bytes)):
            raise Exception(f"Conversion error of {record}")
        ...
        #     DEFAULT_TEXT_PROCESSED_COLLECTION,
        #     record.Title + record.Date + str(record._id),
        # )
    ...
    def run(self, message_data: list[Article]):

        article_data = (
            pull_mongo_data(self.in_col, self.out_col, "Article")
        ...
        try:
            ...
            raise Exception(f"NERTextProcessor: {e}")
