...
def retrieve_latest_news(kw: str, kw_en: str, ln: str):
    next_token = None
    ...
    try:
        while True:
            ...
            articles.extend(
                [Article(**create_article_record(a, kw_en)) for a in res["results"]]
            )
            if not next_token:
                break
        return articles
    ...
    with ThreadPoolExecutor(max_workers=4) as exec:
        for kw in keywords:
            for ln in countries.values():
                future = exec.submit(retrieve_latest_news, translate_word(kw, ln), kw, ln)
