...
from rss_feed import preprocess_compstat
...
def retrive_item1_biz(str1):
    ...
    locations = {}
    if list2 == []:
        # logger.info(f"NO item1 business")
        return ""
    else:
        if list1 == []:
            # logger.info(f"NO item1 business")
...
def retrive_item1a_rf(str1):
    ...
    locations = {}
    if list2 == []:
        # logger.info(f"NO item1a risk factors")
        return ""
    else:
        if list1 == []:
            # logger.info(f"NO item1a risk factors")
...
def run(doc):
    try:
        ...
        str1 = headerclean(content)
        str1 = ASCII_section(str1)
        str1 = remove_special(str1)
        str1 = remove_xbrl(str1)
        str1 = remove_newline(str1)

        item1 = retrive_item1_biz(str1)
        item1a = retrive_item1a_rf(str1)
        item7 = retrive_item7_mda(str1)
        ...
        if item1 == "":
            dummy_item1 = 0
            logger.info(f"NO item1 business for CIK: " + str(cik) + "FILING DATE: " + str(filing_date))
        if item1a == "":
            dummy_item1a = 0
            logger.info(f"NO item1a risk factors CIK: {cik} FILING DATE {filing_date}")
        if item7 == "":
            dummy_item7 = 0
            logger.info(f"NO MD&A CIK: {cik} FILING DATE {filing_date}")

        _id = utils.create_id(cik, filing_date, type)
        ...
        'fyear':doc['fyear'],
        'fmonth': doc['fmonth'],
        # 'fyear_fqtr': doc['fyear_fqtr'],
        'type': type,
        'dummy_item1': dummy_item1,
        'dummy_item1a': dummy_item1a,
        'dummy_item7': dummy_item7
        }
...
if __name__ == '__main__':
    ...
    dbs_name = "AIDF_NLP_Capstone_Temp"
    db = utils.connect_db(dbs_name)
    if db is None:
        logger.error("Can't connect to the database")
        exit(1)
    collection = db['Level1_10K_rss_feed']
    
    # List of filings with items already extracted:
    db_level2 = db["Level2_10K_rss_feed"]
    extracted_filings = set(db_level2.distinct("_id")) 

    # Only extract the filings where items have not been extracted
    documents = list(collection.find({"_id": {"$nin": list(extracted_filings)}}))

    if len(documents) == 0:
        logger.info("No new secfilings which require Item Extraction")
        exit()

    with parallel_backend('threading', n_jobs=15):
        results = Parallel(n_jobs=15)(delayed(run)(doc) for doc in tqdm(documents))
    ...
    for result in results:
        if result is not None:
            ...
            results_summary_list.append(result[1])


    if results_list:
        utils.insert_db(results_list, 'Level2_10K_rss_feed', dbs_name)
    if results_summary_list:
        utils.insert_db(results_summary_list, 'Level2_10K_rss_feed_summary', dbs_name)
